...
@section('title')
<x-breadcrumb :indexPage="'Bakancslista'" :pageTitle="$goal->order . '. ' . $goal->title"
    :indexPageRoute="'goal.index'"></x-breadcrumb>
<div class="w-full bg-sky-100 p-4">
    <h1 class="mb-3 text-left font-serif text-2xl font-bold">{{ $goal->title . ' módosítása'}}</h1>
</div>
@endsection
                ...
                <form action="{{ route('goal.update', $goal->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="mb-5 block">
                        ...
                        <textarea
                            ...
                            id="description" name="description" rows="5" placeholder="min. 10, max. 512 karakter"
                            max="512">{{ $goal->description ?? old('description') }}</textarea>
                    ...
                    <div class="mb-5 block">
                        <div class="form-check">
                            <input
                                ...
                                type="checkbox" value="1" {{ $goal->completed || old('completed') ? ' checked' : '' }}
                            id="completed"
                            name="completed">
