...
<div class="flex justify-between" {{ $attributes }}>
    <div class="mx-3 inline-flex items-center p-1">
        <i class="fas fa-bars cursor-pointer pr-2 text-white transition duration-500"
    ...
    <div class="w-30 flex flex-row items-center p-1">
        ...
        <img onclick="profileToggle()" class="inline-block h-8 w-8 cursor-pointer rounded-full"
            src="{{ Auth::user()->sex === 'male' ? $avatarMale : $avatarFemale }}"
            alt="{{ $userName  }}">
        <a href="#" onclick="profileToggle()"
            class="hidden p-2 text-white no-underline md:block lg:block">
            {{ $userName }}</a>
        <div id="ProfileDropDown"
            class="absolute top-0 right-0 mt-12 mr-1 hidden rounded bg-white shadow-md">
            <ul>
                <ul>
                    <li>
                        <a role="button"
                            href="{{ auth()->id() ? route('user.show', auth()->id()) : route('user.index') }}"
